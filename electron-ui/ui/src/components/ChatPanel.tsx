...
export function ChatPanel({ activeMeetingIds, charCount, onChat, templates, persistedMessages, onSendMessage, onClearMessages, onSaveAsThread, onSourceClick, showIncludeAssets, availableNotes, contextBanner, disableInput }: ChatPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden min-w-0">
      ...
      {contextBanner && (
        <div data-testid="context-banner" className="px-4 py-2 text-xs border-b border-border flex items-center gap-2" style={{ backgroundColor: "#fef8f2", color: "#92400e" }}>
          ...
          {contextBanner.onAction && contextBanner.actionLabel && (
            <button
              onClick={contextBanner.onAction}
              className="ml-auto text-xs underline underline-offset-2 bg-transparent border-none cursor-pointer"
              style={{ color: "#92400e" }}
            >
        ...
        <div className="flex gap-2">
          <div className="flex-1 flex flex-col gap-1.5">
            <textarea
              ...
              className="w-full resize-none rounded-md px-3 py-2 text-sm bg-input text-foreground border border-border outline-none disabled:opacity-50 disabled:cursor-not-allowed"
            />
