...
7. **UI component wiring completeness** *(TypeScript + React projects only)*: For every new interactive component a user is expected to trigger (dialog, panel, action button), verify the plan contains a burst that wires it into the app entry point (`App.tsx` or the relevant page composer). A component built and tested in isolation is not a working feature. The wiring burst must: import the component, manage its open/close state, pass real callbacks, and fetch any prerequisite data needed to determine visibility (e.g. lineage, feature flags). If any new component has no corresponding wiring burst, flag it as **blocking**.
