...
7. **Reachability**: Every feature must be reachable. For each new capability, verify the plan has a burst that connects it to every entry point through which a user or caller is expected to access it. An untriggerable implementation is incomplete, not done.
