...
| `health-banner.spec.ts` | E2E coverage for the SystemHealthBanner component: healthy state (no banner), critical state (red banner with provider/hint/count via `page.route()` intercept), banner position above workspace, gray "unreachable" banner on network abort, and dismiss flow (acknowledge API called, banner hidden). Uses Playwright route intercepts rather than live error seeding for determinism. |
| `health-cli.spec.ts` | E2E coverage for `mti health` CLI commands against a live API server: `mti health status --json` parses as valid `HealthStatus` JSON, human-readable output contains `Status:` line, `mti health acknowledge --json` returns `{ ok: true }`, exit code 0 on success. Runs `cli/mti/bin/mti.ts` via `node --import tsx/esm` with `MTI_BASE_URL` and `MTI_TOKEN` from env. |
