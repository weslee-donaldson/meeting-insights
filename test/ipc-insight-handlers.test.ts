...
describe("insight IPC handlers", () => {
  ...
  it("handleListInsights returns insights for a client", () => {
    handleCreateInsight(db, { client_name: "Acme", period_type: "day", period_start: "2026-03-01", period_end: "2026-03-01" });
    const insights = handleListInsights(db, "Acme");
    expect(insights.length).toBeGreaterThanOrEqual(2);
    expect(insights.every((i) => i.client_name === "Acme")).toBe(true);
  });

  it("handleUpdateInsight updates fields", () => {
    const insight = handleCreateInsight(db, { client_name: "Acme", period_type: "month", period_start: "2026-03-01", period_end: "2026-03-31" });
  ...
  it("handleDeleteInsight removes the insight", () => {
    const insight = handleCreateInsight(db, { client_name: "Acme", period_type: "day", period_start: "2026-03-05", period_end: "2026-03-05" });
    handleDeleteInsight(db, insight.id);
    const list = handleListInsights(db, "Acme");
  ...
  it("handleGetInsightMessages returns empty array for new insight", () => {
    const insight = handleCreateInsight(db, { client_name: "Acme", period_type: "day", period_start: "2026-03-01", period_end: "2026-03-01" });
