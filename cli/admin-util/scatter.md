# cli/admin-util/ -- Operator and developer scripts that drive core/ directly
...
| File | Purpose |
|------|---------|
| `shared.ts` | Shared CLI configuration loader. Reads `.env.local`, exports `loadCliConfig()` returning DB path, vector path, LLM provider settings, and related env vars. Used by all other scripts and by `local-service/main.ts`. |
| `run.ts` | Main ingestion script (`pnpm process`). Loads `.env.local`, initialises the DB and vector DB, loads the ONNX model, then calls `processNewMeetings` with both webhook (`data/webhook-rawtranscripts/`) and manifest (`data/raw-transcripts/`) sources. Webhook files are processed first; their meeting IDs feed into the manifest dedup set so the same meeting is never processed twice. Prints per-file progress with client assignment and elapsed time. Writes a full event log to `logs/run-<timestamp>.json`. |
| `setup.ts` | First-time and re-initialisation script (`pnpm setup`). Creates the SQLite DB, runs migrations, seeds clients from `config/clients.json`, and ensures LanceDB tables exist. When `MTNINSIGHTS_LLM_PROVIDER=local`, validates Ollama server reachability and pulls the configured model if absent. Run this once before the first `pnpm process`. |
...
| `reset.ts` | Full pipeline reset (`pnpm clear`). Deletes the SQLite DB and LanceDB directory, then moves all files from `data/processed/` and `data/failed-processing/` back to `data/raw-transcripts/` so they can be reprocessed. Leaves client config and model files untouched. |
| `all-items-dedupe.ts` | Cross-meeting item deduplication (`pnpm all-items-dedupe`). Subcommands: `run` (dedupe) and `clear` (reset associations). Uses semantic (cosine similarity) and string (Jaro-Winkler) thresholds. Supports `--dry-run`, `--deepscan`, `--last-day`, `--date=`. |
| `import-external.ts` | External transcript converter (`pnpm import-external`). Converts non-Krisp transcripts from `data/external-transcripts/` into the Krisp format under `data/raw-transcripts/` with a manifest entry, ready for `pnpm process`. |
| `test-ollama.ts` | Ollama connectivity check (`pnpm test-ollama`). Verifies the configured Ollama server is reachable, lists available models, confirms the target model is present, and sends a minimal test prompt to validate inference end-to-end. Use this to diagnose local LLM issues before running `pnpm setup` or `pnpm process`. |
