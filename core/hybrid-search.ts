...
export async function hybridVectorSearch(
  ...
  const [meetingResults, featureResults, itemResults] = await Promise.all([
    searchMeetingsByVector(vdb, vec, { limit: options.limit, client: options.client, client_id: options.client_id, date_after: options.date_after, date_before: options.date_before }),
    hasFeatures
