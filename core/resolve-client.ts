...
export function resolveClient(db: Database, clientParam: string, tenantId?: string): ClientRow | null {
  ...
  if (tenantId) {
    return (db.prepare("SELECT * FROM clients WHERE name = ? AND tenant_id = ?").get(clientParam, tenantId) as ClientRow) ?? null;
  }
  return (db.prepare("SELECT * FROM clients WHERE name = ?").get(clientParam) as ClientRow) ?? null;
}
